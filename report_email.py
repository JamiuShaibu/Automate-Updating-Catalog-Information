...
def pdf_body(input_for, desc_dir):
    ...
    for item in os.listdir(desc_dir):
        ...
        with open(filename) as f:
            ...
            print(res)
            print(wt)
    new_obj = ""  # To initialize the object
    # To call values from two lists one by one.
...
if __name__ == "__main__":
    ...
    generate_report('/tmp/processed.pdf', title,
                    pdf_body('pdf', description_directory))  # calling the report function from custom module
    ...
    msg = generate_email("automation@example.com", "<username>@example.com".format(user),
                         ...
                         "/tmp/processed.pdf")  # structuring email and attaching the file. Then sending the email, using the cus$
    send_email(msg)
